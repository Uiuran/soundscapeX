...
# Plot
  ...
  tf.train.write_graph(frozen_graph, "model", "./conv.pb", as_text=False)

# Save reference optimized filtered signal and an one db louder version of the original audio
  audio.wavwrite(signal_out_value[0,:,0],'/content/signout.wav', fs=fs, enc=enc)
  audio.wavwrite( 10.0*tensor_data[0,:,0],'/content/signin.wav', fs=fs, enc=enc)
